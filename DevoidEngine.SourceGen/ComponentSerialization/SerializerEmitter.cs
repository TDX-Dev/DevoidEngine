            ...
            var fields = component
                ...
                .Where(f =>
                    ...
                    (
                        ...
                        (
                            !f.Type.ToDisplayString().StartsWith("DevoidEngine.Core") &&
                            !f.Type.ToDisplayString().StartsWith("DevoidEngine.Rendering") &&
                            !f.Type.ToDisplayString().StartsWith("DevoidEngine.UI")
                        )
                    )
                ).ToArray();
                    ...
                    try
                    {
                        ...
                        component.{{fieldName}} =
                            guid_{{fieldName}} == Guid.Empty
                                ? default
                                : DevoidEngine.Core.Engine.Instance.AssetManager.Load<{{type}}>(guid_{{fieldName}});
                    }
                    catch (Exception e)
                    {
