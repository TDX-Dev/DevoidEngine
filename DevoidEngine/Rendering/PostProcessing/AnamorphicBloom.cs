...
namespace DevoidEngine.Rendering.PostProcessing
{
    public class AnamorphicBloomPass : PostProcessPass
    {
        ...
        {
            Material downsampleMat = new(Shader.FromDescriptorFile(Engine.GraphicsDevice, Path.Combine(Engine.BasePath, "Content/DevoidShaderDescriptors/bloom_downsample_pass.dsd")));
            Material upsampleMat = new(Shader.FromDescriptorFile(Engine.GraphicsDevice, Path.Combine(Engine.BasePath, "Content/DevoidShaderDescriptors/bloom_upsample_pass.dsd")));

            downsampleMat.SetDefine("ANAMORPHIC");
            upsampleMat.SetDefine("ANAMORPHIC");

            prefilterMaterial = new(new(Shader.FromDescriptorFile(Engine.GraphicsDevice, Path.Combine(Engine.BasePath, "Content/DevoidShaderDescriptors/bloom_process_pass.dsd"))));
            downsampleMaterial = new(downsampleMat);
            upsampleMaterial = new(upsampleMat);
            ...
            {
                bloomMipNames.Add($"PP_ANAMORPHICBLOOM_{i}");
                upsampleMipNames.Add($"PP_ANAMORPHICBLOOM_UP_{i}");
            }
        }
        ...
        public override void Execute(PostProcessContext ctx)
        {
            ...
            ctx.SetTexture("AnamorphicBloom", upsampleMipList[0].Texture);
        }
